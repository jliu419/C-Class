...
void StudentList::displayBelow(const double gline) const
{
    ListNode * pCur; // moving through the linked list
    
    pCur = head -> next; // set the starting point to be in the beginning of the list
    
    int count = 0;
    
    cout << "Student with Lower gpa than " << gline << ": " << endl;
    while (pCur != NULL) // when there are still space for list.
    {
        if ((pCur->stu).gpa <= gline)
        {
            cout << (pCur->stu).name << " " << (pCur -> stu).gpa << endl;
            count++;
        }
